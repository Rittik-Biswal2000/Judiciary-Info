document.querySelector("#btn_missing").addEventListener('click', e => {
    ...
    const Status = document.querySelector("#MissingSince").value;

    async function writeDB() {

        showLoading();
        var newPostKey = await firebase.database().ref().child('Wanted_List').push().key;
        await firebase.database().ref('Wanted_List/' + newPostKey).set({
